...
import com.prokarma.com.repository.AuthorRepository;
...
public class DataLoader {
	...
	private void createPosts(Author author) {
		...
		try {
			...
			futureDate = new SimpleDateFormat("MM/dd/yyyy h:mm:ss a").parse("12/31/2016 12:00:00 PM");
		...
		Post groovy = new Post("Groovy is next!");
		groovy.setSlug("groovy-is-next");
		groovy.setTeaser( getTeaser() );
		groovy.setBody( getBody() );
		groovy.setPostedOn( futureDate );
		groovy.setKeywords( getGrailsKeywords() );
		groovy.setActive(true);
		groovy.setAuthor( author );
		postRepository.save(groovy);
